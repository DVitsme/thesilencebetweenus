...
- `/supporters` (`app/supporters/page.tsx`) — the designed **Founding Supporters wall**, built from `handoff/10`. Server page computes the live counts (total / patrons+partners / founding year) + renders the dark bridge CTA; the search/tier-filter/show-more block is the `"use client"` island `components/site/supporters/wall.tsx`. Roster now reads **Cloudflare D1** (`lib/db/supporters.ts`; the webhook persists per doc 08 §7) — page is `force-dynamic`; **privacy:** credit-name only, never email. V1 seed `db/seed-supporters.sql` made the wall look complete for review (cleared at go-live via `DELETE … LIKE 'seed:%'`). **Real roster (2026-06-16):** Kevin's confirmed supporters live in `db/founding-supporters.sql` (`founding:` ids — survive the `seed:` purge; `pnpm db:founding[:local]`); local D1 had the samples cleared and the roster is live on **remote + local D1** (`founding:0001–0022`). **22 real names as of 2026-07-10:** Dr. Tanya Lea (Partner) + 21 supporters. Add/rename via `INSERT`/`UPDATE` on D1 by `founding:` id, keeping `db/founding-supporters.sql` in sync — the file holds the `INSERT OR IGNORE` block **plus** trailing `UPDATE`s for renames (OR IGNORE cannot change an existing row; e.g. `founding:0006` → "Osakwe & Mari Mattocks"). The wall is `force-dynamic`, so a remote-D1 write is **live instantly, no deploy**. Filter behavior ported verbatim from the mockup (patrons hide under Partner/Supporter; search narrows both). CTAs link to `/give`.
- `/contact` (`app/contact/page.tsx`) — the designed **Contact** page, built from `handoff/11`. Server page = hero + `<Rule>` + a two-col grid (form left, direct-details aside right); the form is the `"use client"` island `components/site/contact/contact-form.tsx` (inquiry chips, validation, inline warm success state). Submit is a resolved **stub** (`TODO(contact-wiring)`: POST `/api/contact` → verify **Cloudflare Turnstile** → Resend to `CONTACT_TO_EMAIL` with the inquiry category in the subject). **Updated 2026-06-16:** the phone line was **removed**, the production location dropped to just **Take 3 Media** (no Cleveland), and the displayed contact email is now **kevin@take3mediallc.com** site-wide (`SITE.email` in `nav-links.ts` + ~12 hardcoded spots across legal/error/canceled/contact/emails, swept by exact-string replace; Instagram `@kcfilmsmedia` + Cash App `$kcfilmsmedia` handles intentionally left). Remaining `TODO(contact-confirm)`: a real **Facebook URL** (still `#`). ⚠️ the *displayed* email is separate from *delivery* — `CONTACT_TO_EMAIL`/`CONTACT_FROM_EMAIL` Cloudflare secrets + a **verified Resend domain** still drive actual sending (update those for take3mediallc.com). Admin note still carries the deferred "Donations…" wording (same as footer).
...
is still the test sender `hello@digitaldog.io` until take3mediallc.com is verified.) ⚠️ Remaining infra:
attach `thesilencebetweenus.film` to the Worker as a **Custom Domain** in Cloudflare (zone must be on
Cloudflare) so it serves off the real URL, not just `*.workers.dev`. Launch checklist: `docs/build-plan.md`.
...
email; **Cloudflare Turnstile** on contact (switched off Google reCAPTCHA 2026-06-19; no Google account). Framing = **rewards crowdfunding** ("support/back/contribute",
...
(currently crossover); trailer/pitch video. *(Tier amounts ✅ confirmed 2026-06-20; Facebook ✅ set;
...
- **▶ PRODUCTION — LIVE on `https://thesilencebetweenus.film` (custom domain attached to the Worker).**
  **Stripe is now LIVE mode (flipped 2026-06-20):** `STRIPE_MODE=live`, live keys + a **live** webhook
  ...
  Facebook URL ✅ set (facebook.com/kcfilmsmedia). Remaining open: the footer/admin "Donations…" wording.
